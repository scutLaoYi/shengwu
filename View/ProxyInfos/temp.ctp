	...
	$.ajax({
		dataType:"HTML",
			...
			url:'<?php echo Router::url(array(
				...
				'action'=>'proxy_list')); ?>/'+provinceIndex,
			data: ({type:'original'}),
...
function flash()
{
	...
	funA(area.options[area.selectedIndex].value);
	//here!
}
...
<div class="actions">
<?php
echo $this->Form->input('product_area', array('id'=>'product_area', 'label'=>'代理地区', 'options'=>$allCountrys,'onchange'=>'funA(this.options[this.selectedIndex].value)'));
